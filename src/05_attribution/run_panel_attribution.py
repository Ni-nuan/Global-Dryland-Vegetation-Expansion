# -*- coding: utf-8 -*-
"""
run_panel_attribution.py

Based on your v2a (logit-scale attribution as MAIN result), with the OLD v1-style
(probability-scale / veg_frac) attribution added as SUPPLEMENTARY outputs.
...
FILE_XCO2    = r"CO2_annual_tif.xlsx"        # -> XCO2 (main spatially heterogeneous background panel)

# Switches
INCLUDE_ET = True
INCLUDE_SWNET = True
INCLUDE_INTERACTION = True   # P90×VPD_resid
...
# VPD_resid settings (minimal-change sensitivity version)
VPD_RESID_INCLUDE_SWNET = True
VPD_RESID_DOUBLE_DEMEAN = True

# Output directory
OUTDIR = r"trend_outputs_xco2_vpd_resid"
...
def pooled_vpd_resid(panel: pd.DataFrame,
    ...
    Optionally apply double-demean (hex + year) before regression.
    Returns a Series aligned to panel.index.
    """
    need = ["VPD90", "Tmean90"] + (["SWnet90"] if include_swnet and "SWnet90" in panel.columns else [])
...
def build_panel(spec: DatasetSpec) -> pd.DataFrame:
    ...
    # Residualized atmospheric demand (minimal-change replacement of raw VPD90)
    ...
    if INCLUDE_INTERACTION:
        panel["P90xVPDresid"] = panel["P90"] * panel["VPD_resid"]
...
def get_covars(include_xco2: bool) -> List[str]:
    covars = ["P90", "VPD_resid", "Tmean90", "SM90"]
    if INCLUDE_ET:
        covars.append("ET90")
    if INCLUDE_SWNET:
        covars.append("SWnet90")
    if INCLUDE_INTERACTION:
        covars.append("P90xVPDresid")
    if include_xco2:
        covars.append("XCO2")
    return covars
...
def run_dataset(spec: DatasetSpec) -> None:
    ...
    runinfo = {
        ...
        "switches": {
            ...
            "include_interaction_P90xVPDresid": INCLUDE_INTERACTION,
            "vpd_used_in_panel": "VPD_resid",
